@extends('layouts.app_layout')

@section('title','All Categories')

@section('content')

<div class="all-categories-main-container">
    <div class="all-categories-main-heading">
        Browse Coupons By Category
    </div>
    <div class="ac-popular-categories-container">
        <div class="ac-popular-categories-heading">
            Popular Categories
        </div>
        <div class="ac-popular-categories-list-container">
            @for($i=1; $i<=12; $i++)
            <div class="ac-popular-category-container">
                <a href="#" class="ac-popular-category-link">
                    <div class="ac-popular-category-logo">
                        <img src="data:image/png;base64,"/>
                    </div>
                    <div class="ac-popular-category-title">Clothing</div>
                </a>
            </div>
            @endfor
        </div>
    </div>
    <div class="all-categories-list-container">
        <div class="all-categories-list-heading">
            Categories List
        </div>
        <div class="all-categories-list">
            <ul>
                @for($i=1; $i<= 40; $i++)
                    <li>
                        <div class="ac-category-title-container">
                            <div class="all-categories-list-bullets">&#9654</div>
                            <div class="ac-category-title"><a href="#">Clothing</a></div>
                        </div>
                    </li>
                @endfor
            </ul>
        </div>
    </div>
</div>

@endsection